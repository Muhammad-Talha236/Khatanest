        ...
        <div style={{ background: 'var(--surface-alt)', border: '1px solid var(--border)', borderRadius: 20, padding: 28 }}>
          ...
          <div style={{ display: 'flex', background: 'var(--surface)', borderRadius: 10, padding: 4, marginBottom: 24 }}>
            {['Login', 'Register'].map(t => (
              <button key={t} onClick={() => setIsRegister(t === 'Register')} style={{
                ...
                background: (isRegister ? t === 'Register' : t === 'Login') ? 'var(--accent-soft)' : 'transparent',
                color: (isRegister ? t === 'Register' : t === 'Login') ? 'var(--accent)' : 'var(--text-muted)',
              }}>{t}</button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              background: 'linear-gradient(135deg, #2ECC9A, #1A7A5C)',
              color: '#000', fontWeight: 800, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer',
              boxShadow: '0 4px 20px rgba(46,204,154,0.3)', opacity: loading ? 0.7 : 1,
