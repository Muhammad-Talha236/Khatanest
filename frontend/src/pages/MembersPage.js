    ...
    <div>
        ...
        {group?.members?.map((m, i) => {
          ...
          return (
            <div key={m._id} style={{
              ...
              border: `1px solid ${color}22`, borderRadius: 16, padding: 20,
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <Avatar name={m.name} size={44} color={color} />
                  <div>
                    <div style={{ fontWeight: 800, color: 'var(--text)', fontSize: 15 }}>{m.name}</div>
                ...
                <span style={{
                  ...
                  padding: '3px 10px', borderRadius: 99, fontSize: 10, fontWeight: 800, letterSpacing: 0.5,
              ...
              {m.role !== 'admin' && (
                <button onClick={() => setRemoveId(m._id)} style={{
                  width: '100%', padding: '8px', borderRadius: 8,
                  background: 'var(--red-soft)', border: '1px solid rgba(255,92,106,0.3)',
                  color: 'var(--red)', fontWeight: 700, cursor: 'pointer', fontSize: 12,
      ...
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add New Member">
        <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 20, lineHeight: 1.5 }}>
          Create a login account for the new member. They can use these credentials to login and view their balance.
        </p>
      ...
      <ConfirmModal isOpen={!!removeId} onClose={() => setRemoveId(null)} onConfirm={handleRemove}
        title="Remove Member" confirmText="Remove" loading={removing}
